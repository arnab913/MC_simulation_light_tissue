% --- Sweep parameters ---
ua_ink = [1,7,30,100];                  % for g= 0.9, us is 10 times us' 
...
for idx = 1:length(ua_ink)
  ...
  for j = 1:length(all_det_r_cm)
    ...
    % fname = sprintf('Ink Tissue_ %d ua ink signal 0.3cm radius.mat',ua_ink(idx));
    % save(fname, 'reflectance', 'all_det_r_cm');
    fname_model = sprintf('ND_ink_%g.mat', ua_ink(idx));
    save(fname_model, 'model');  % saves model.G, model.MC, etc.

    % (Optional) Display progress
    fprintf('Finished ua = %d  cm^-1\n',ua_ink(idx));
end
...
for k = 1:numel(files)
    ...
    if ~isempty(splitnum)
        ua = str2double(splitnum{1}); % Assumes first number is us
